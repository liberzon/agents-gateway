...
class TestGoogleContactsProvider(unittest.TestCase):
    def test_create_success(self):
        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(200, json={"resourceName": "people/c123", "etag": "%EgQBAj0="})

        http_client = make_mock_client(handler)
        provider = GoogleContactsProvider(ProviderConfig(), http=http_client)

        c = Contact(given_name="Alex", emails=["alex@example.com"])
        res = provider.create_contact(token="t", contact=c)
        self.assertEqual(res.status, "success")
        self.assertEqual(res.resource_name, "people/c123")
